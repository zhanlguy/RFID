...
for b = 1:length(jgrid)
    for a = 1:length(igrid)
        ...
        phian1(b,a) = atan(real(Rx1)/y);
        phian2(b,a) = atan((real(Rx2))/y);
        phian3(b,a) = atan((real(Rx3))/y);
    end
end
...
PLMAP3 = cell2mat(PL3).*phase2.*(signal3);

PLMAP11 = phase1.*(signal1);
PLMAP22 = phase2.*(signal2);
PLMAP33 = phase2.*(signal3);


MAP=[];
for ii = 1:b
    for jj = 1:a
        if D1(ii,jj) < D2(ii,jj)|| D1(ii,jj) < D3(ii,jj) 
            DMap(ii,jj) = PLMAP1(ii,jj)./PLMAP3(ii,jj)+PLMAP2(ii,jj)+noise;
            %OMap(ii,jj) = PLMAP11(ii,jj)./PLMAP22(ii,jj)+noise;
        elseif D2(ii,jj) < D1(ii,jj)|| D2(ii,jj) < D3(ii,jj)
            DMap(ii,jj) = PLMAP2(ii,jj)./PLMAP3(ii,jj)+PLMAP1(ii,jj)+noise;
           % OMap(ii,jj) = PLMAP22(ii,jj)./PLMAP11(ii,jj)+noise;
        elseif D3(ii,jj) < D2(ii,jj)|| D3(ii,jj) < D1(ii,jj)
